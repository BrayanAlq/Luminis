...
cv::Mat ecualizarHistogramaGrises(const cv::Mat &imgGray)
{
    ...
    if (denom == 0)
        denom = 1; // evitar divisi�n por cero
...
int ejecutarModoSerial()
{
    ...
    cout << "=== Ecualizaci�n de Histograma (Versi�n Serial) ===" << endl;
    ...
    cout << "Proceso completado. Im�genes guardadas en: " << rutaSalida << endl;
    return 0;
}

int main() {
 return ejecutarModoSerial();
